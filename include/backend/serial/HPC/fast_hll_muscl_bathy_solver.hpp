...
class FastHLLMUSCLBathySolver {
  public:
    ...
    void run() {
        const double dt_stable = fast_hll_muscl_bathy::compute_stable_dt(gv_, h_.data(), hu_.data(),
        ...
        fill_io_state();
        writer_.write_snapshot(io_state_, time, dt_, "HLL", "MUSCL", "SSPRK3", "Reflecting Walls",
                               bathymetry_name());
