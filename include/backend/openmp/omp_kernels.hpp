...
namespace fast_hll_muscl_bathy_omp {
...
#pragma omp parallel for collapse(2) schedule(static) reduction(max : max_speed)
    for (int i = g.nG; i < g.nG + g.Nx; ++i) {
        for (int j = g.nG; j < g.nG + g.Ny; ++j) {
...
#pragma omp parallel for collapse(2) schedule(static) reduction(max : nonfinite)
    for (int i = g.nG; i < g.nG + g.Nx; ++i) {
        for (int j = g.nG; j < g.nG + g.Ny; ++j) {
            const int k = idx(i, j, g.stride);
            if (h[k] < h_floor) {
    ...
    if (nonfinite) {
        throw std::runtime_error("non-finite state detected in OpenMP fast solver");
    }
}
